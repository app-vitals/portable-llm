...
def anthropic_examples():
    ...
    response = client.messages.create(
        ...
        messages=[{"role": "user", "content": "Get me a weather report for Paris"}],
...
def openai_examples():
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a weather assistant. Use tools to get weather data, then provide a structured response.",
        },
        {
            "role": "user",
    ...
    while True:
        ...
        if response.choices[0].finish_reason == "stop":
            ...
            weather_report = WeatherReport.model_validate_json(
        ...
        if response.choices[0].finish_reason == "tool_calls":
            ...
            for tool_call in response.choices[0].message.tool_calls:
                ...
                if function_name == "get_current_weather":
                    ...
                    print(
                        f"Tool result for {function_args.get('location')}: {function_response}"
                    )
...
def litellm_examples():
    ...
    print(
        "Demonstrating weather agent loop that terminates when 'weather_report' tool is called..."
    )
    ...
    for model in ["gpt-4o-mini", "claude-3-haiku-20240307"]:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a weather assistant. First get weather data for locations, then generate a structured weather report.",
            },
            {
                "role": "user",
            ...
            if response.choices[0].message.tool_calls:
                ...
                for tool_call in response.choices[0].message.tool_calls:
                    ...
                    if function_name == "get_current_weather":
                        ...
                        print(
                            f"Weather data for {function_args.get('location')}: {function_response}"
                        )
                        ...
                        print("Weather report tool called - agent loop complete!")
